...
bool VulkanGraphicsPipeline::init(VulkanResources* resources, const VkDevice& device, const CreateGraphicsPipelineOptions& desc)
{
    ...
    //// Pipeline layout
    const VulkanPipelineLayout* vulkan_layout = resources->pipeline_layouts.get(desc.layout);
    if (vulkan_layout == nullptr) [[unlikely]]
    {
        CLAY_LOG_ERROR("Failed to get pipeline layout!");
        return false;
    }
    ...
    create_info.layout                       = vulkan_layout->layout;
...
void VulkanResources::destroy(const VkDevice& device, const VmaAllocator& vma_allocator)
{
    ...
    samplers.each([&](VulkanSampler& s) { vkDestroySampler(device, s.sampler, nullptr); });
